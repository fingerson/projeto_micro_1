...
pedido getRequest() //Função que recebe os dados do bluetooth e retorna uma struct de formulário, é executada a cada loop e retorna um formulário vazio se não terminou de receber a mensagem
{
  ...
  else if(input=='e') // e é o caracter que indica que o comando chegou ao fim, com isso o programa decifra o comando.
  {
    ...
    else if (command[0]=='s') // caso o comando seja complexo (configuração) a função retorna um formulário completo
    {
      ...
      i++;
      request.estadoInicial=charToInt(command[i]);
    }
...
{
  Serial.print("r"); // Indicador de mensagem de retorno.
  Serial.print(ID); // Informa a qual processo pertencem as informações.
  Serial.print(".");
  Serial.print(Proc->estaAtivo); // Informa se o processo está ativo.
  Serial.print(".");
  Serial.print(Proc->estadoAtual); // Informa se a saída do processo está ligada ou desligada
  Serial.print(".");
  Serial.print(Proc->condLigar); // Informa a condição de ativação da saída
  Serial.print(".");
  Serial.print(Proc->condDesligar); // Informa a condição de desativação da saída
  Serial.print(".");
  Serial.print(Proc->valorLigar); // Informa o valor necessário para ativar a saída
  Serial.print(".");
  Serial.print(Proc->valorDesligar); // Informa o valor necessário para desativar a saída
  Serial.print(".");
  Serial.print(Proc->valorAtual); // Informa o valor atual
  Serial.println("e"); // Indicador de fim de mensagem
...
void calcularMudanca(struct processos *Proc,int ID) // calcula as condições e realiza a mudança de estado dos processos
{
  if(Proc->estaAtivo) // verifica se o processo está habilitado
  {
    ...
    if(Proc->estadoAtual == 0 && Proc->valorAtual>=Proc->valorLigar) // verifica se ocorre a mudança de estado
    {
        Proc->estadoAtual=1;
        Proc->valorAtual=0;
        Proc->ultimaInteracao=millis();
    ...
    if(Proc->estadoAtual == 1 && Proc->valorAtual>=Proc->valorDesligar) // verifica se ocorre a mudança de estado
    {
        Proc->estadoAtual=0;
        Proc->valorAtual=0;
        Proc->ultimaInteracao=millis();
